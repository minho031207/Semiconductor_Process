...
그러나 Halo 조건이 과도하면 채널 내부의 전위 장벽이 높아져 Ion이 감소할 수 있습니다. 따라서 누설전류 감소와 구동 전류 보존 사이의 trade-off를 고려해야 합니다.

---

## 6.2 Halo Dose Sweep

Halo Energy를 15 keV로 고정하고 Dose를 넓은 범위에서 sweep하여 개선 가능한 구간을 탐색했습니다.
...
### Dose Sweep Interpretation

- 낮은 Halo Dose에서는 punch-through 억제 효과가 충분하지 않았습니다.
- `1.0×10¹³ ~ 2.0×10¹³ cm⁻²` 구간에서 Ion/Ioff 개선 가능성을 확인했습니다.
- `5.0×10¹³ cm⁻²` 이상에서는 Ion이 급격히 감소하고 SS가 악화되었습니다.
- 과도한 Halo 도핑은 채널 전위 장벽을 증가시켜 구동 전류를 제한하는 것으로 해석했습니다.

따라서 `1.0×10¹³ ~ 2.0×10¹³ cm⁻²` 구간을 정밀 탐색 범위로 선정했습니다.

---

## 6.3 Halo Energy Sweep

Halo Dose를 `1.5×10¹³ cm⁻²`로 고정하고 Energy를 25~35 keV 범위에서 sweep했습니다.
...
35 keV는 가장 높은 Ion/Ioff를 나타냈지만, Ion이 감소하고 SS가 증가하는 경향을 보였습니다.

따라서 Ion/Ioff만을 최대화하지 않고, **Ion·Ioff·SS 사이의 균형이 가장 안정적인 26 keV**를 최종 조건으로 선정했습니다.
...
### Improvement after Halo Optimization

| Metric | Baseline | Halo Optimized |
| :--- | ---: | ---: |
| Ion/Ioff | 23 | 867 |
| Subthreshold Swing | 1015.2 mV/dec | 838.3 mV/dec |
| Ioff | 2.81 × 10⁻⁵ A/μm | 4.07 × 10⁻⁷ A/μm |

Halo 최적화만으로 Ion/Ioff가 약 37배 향상되었으며, 이후 LDD 최적화를 수행할 수 있는 개선된 공정 기준을 확보했습니다.
...
├── README.md
├── automotive-nmos-final-report.pdf
├── automotive-nmos-presentation.pdf
├── 온도상승에따른성능변화.png
├── 베이스라인1.png
├── 베이스라인2.png
├── 최종1.png
...
> 현재 command 파일을 저장소 루트에 업로드하는 경우에는 위 `source/` 경로를 실제 파일 위치에 맞게 수정해야 합니다.
